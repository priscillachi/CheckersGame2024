...
public class App extends PApplet {
    ...
    private int currentPlayerCode = 0;
    ...
    */
    ...
    public void mousePressed(MouseEvent e) {
		...
		if (clicked.getPiece() != null && clicked.getPiece().getColour() == currentPlayer) {
            ...
            CheckersPiece toMove = currentSelected.copyPiece();

            if (selectedCells == null) {
                return;
            }
            
            for (Cell j : selectedCells) {

                if (clicked.equals(j) && toMove != null) {
                    cellChosen = clicked;

                    clicked.setPiece(toMove);
                    toMove.setPosition(clicked);
                    ...
                    // change current selected piece to null
                    board[currentSelected.getPosition().getY()][currentSelected.getPosition().getX()].removePiece();
            ...
            if (cellChosen != null) {
                try {
                    int jumpedOver_x = Integer.parseInt(String.valueOf((currentSelected.getPosition().getX() + cellChosen.getX())/2));
                    int jumpedOver_y = Integer.parseInt(String.valueOf((currentSelected.getPosition().getY() + cellChosen.getY())/2));

                    if (board[jumpedOver_y][jumpedOver_x].getPiece().getColour() != toMove.getColour()) {
                        board[jumpedOver_y][jumpedOver_x].getPiece().capture();
                    }

                } catch (Exception noJump) {
                    ;
                }
            }
    ...
    */
    public void draw() {
        ...
        HashSet<Cell> cellsToHighlight = new HashSet<Cell>(); //initiate HashSet to add available moves later
        ...
        for (int i = 0; i < board.length; i++) {
            for (int i2 = 0; i2 < board[i].length; i2++) {
                if (currentSelected != null && board[i][i2].getPiece() == currentSelected) {
                    ...
                    for (Cell highlightBlue : cellsToHighlight) {
                        this.setFill(2, currentPlayerCode);
                        this.rect(highlightBlue.getX()*App.CELLSIZE, highlightBlue.getY()*App.CELLSIZE, App.CELLSIZE, App.CELLSIZE);
        ...
        for (int i=0; i<board.length; i++) {
            for (int i2=0; i2<board[i].length; i2++) {
                if (board[i][i2].getPiece()!= null && board[i][i2].getPiece().getColour() == 'w') {
                    hasWhite = true;
                } else if (board[i][i2].getPiece()!= null && board[i][i2].getPiece().getColour() == 'b') {
                    hasBlack = true;
                }
            }
        }
